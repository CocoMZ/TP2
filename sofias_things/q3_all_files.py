...
# for key in keys:
sigma_y2 = np.mean(np.abs(y_real_dict["exp"])**2)  # E{|y|^2}

U = np.fft.fft(u_real_dict["exp"])  # FFT of signal
Y = np.fft.fft(y_real_dict["exp"])
...
# '''
H_hat = np.fft.fft(wo_tab[0], n=len(u_real_dict["music"]))
Y = np.fft.fft(u_real_dict["music"])  # putting signal into frequency domain
